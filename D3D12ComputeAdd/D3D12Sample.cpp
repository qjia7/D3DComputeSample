...
void D3D12Sample::LoadAssets()
{
    ...
    // Create a constant buffer.
    {
        ...
        m_constantBufferData.M = m_dataSize;
...
void D3D12Sample::RunCompute()
{
    ...
    for (int it = 0; it < m_computeCount; it++)
    {
        ...
        m_commandList->Dispatch(m_dataSize / (m_workGroupSizeX * m_componentSize), 1, 1);
    ...
    for (UINT i = 0; i < m_computeCount; i++)
    {
        ...
        const UINT64 gpuTimeUS =  (timeStampDelta * 1000 * 1000) / m_timestampFrequency;
    ...
    UINT64 outputBufferSize = m_dataSize * sizeof(float);
